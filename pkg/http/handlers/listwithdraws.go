...
func (lts *ListWithdrawsService) ListWithdrawsHandler(w http.ResponseWriter, req *http.Request) {
	...
	lts.InvokeListWithdraws(&RES, symbol, tokenType, limitInt)
...
func (lts *ListWithdrawsService) InvokeListWithdraws(RES *ListWithdrawsHandlerResponseMap, symbol, tokenType string, limit int) {
	rpcConfigCount := 0
	...
	for _, curr := range config.CURRRPC {
		...
		for _, rpcConfig := range curr.RpcConfigs {
			...
			go func(currencyConfig cc.CurrencyConfig, rpcConfig rc.RpcConfig) {
				...
				if err != nil {
					logger.ErrorLog(" - ListWithdrawsHandler lts.moduleServices.GetModule err: " + err.Error())
				...
				if err != nil {
					logger.ErrorLog(" - ListWithdrawsHandler (*lts.moduleServices)[SYMBOL].ListWithdraws(rpcConfig, limit) Error: " + err.Error())
					_RES.Error = err.Error()

				} else {
					logger.Log(" - InvokeListWithdraws Symbol: " + currencyConfig.Symbol + ", RpcConfigId: " + strconv.Itoa(rpcConfig.Id) + ", Host: " + rpcConfig.Host)
