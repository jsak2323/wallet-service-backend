...
func (s *cronService) HealthCheckHandler() {
	...
	getBlockCountService := h.NewGetBlockCountService(s.moduleServices, s.systemConfigRepo)

	// after 11 or more minutes, save health check to db. otherwise, only ping
	lastUpdatedTime, _ := s.healthCheckRepo.GetLastUpdatedTime()
	...
	maintenanceList, err := h.GetMaintenanceList(ctx, s.systemConfigRepo)
	if err != nil {
		...
		for resTokenType, resRpcConfigs := range mapTokenType {
			for _, resRpcConfig := range resRpcConfigs {
				...
				if isPing { // if ping, only check if blockount is 0
					if nodeBlockCount <= 0 {
						s.sendNotificationEmails(ctx, resRpcConfig, -1)
					}
				...
				if !isBlockCountHealthy && config.FirstHealthCheck { // if not healthy, send notification emails
					s.sendNotificationEmails(ctx, resRpcConfig, blockDiff)
				}

				config.FirstHealthCheck = true

				if !isPing {
					s.saveHealthCheck(ctx, resRpcConfig.RpcConfig.RpcConfigId, nodeBlockCount, blockDiff, isBlockCountHealthy)
				}
			}
		}
	}
...
func (s *cronService) saveHealthCheck(ctx context.Context, rpcConfigId int, blockCount int, blockDiff int, isBlockCountHealthy bool) error {
	existingHealthCheck, err := s.healthCheckRepo.GetByRpcConfigId(rpcConfigId)
	if err != nil {
	...
	if existingHealthCheck.Id == 0 { // does not exist, create a new one
		...
		err := s.healthCheckRepo.Create(&newHealthCheck)
		if err != nil {
		...
		err := s.healthCheckRepo.Update(&newHealthCheck)
		if err != nil {
			err = errs.AddTrace(err)
		} else {
...
func (s *cronService) sendNotificationEmails(ctx context.Context, res h.GetBlockCountRes, blockDiff int) {
