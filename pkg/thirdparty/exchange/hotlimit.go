...
func (r *exchangeHotLimitRepository) GetBySymbol(symbol string) (result domain.HotLimit, err error) {
	...
	if cmp, err := util.CmpCurrency(volume, "500000000"); err != nil {
		return domain.HotLimit{}, err
	} else if cmp == -1 { // <500jt
	...
	if cmp, err := util.CmpCurrency(volume, "1000000000"); err != nil {
		return domain.HotLimit{}, err
	} else if cmp == -1 { // <1m
	...
	if cmp, err := util.CmpCurrency(volume, "2500000000"); err != nil {
		return domain.HotLimit{}, err
	} else if cmp == -1 { // <2.5m
	...
	if cmp, err := util.CmpCurrency(volume, "5000000000"); err != nil {
		return domain.HotLimit{}, err
	} else if cmp == -1 { // <5m
