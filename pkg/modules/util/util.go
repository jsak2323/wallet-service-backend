...
func IsBlockCountHealthyFallback(ctx context.Context, service modules_m.ModuleService, nodeBlockCount int, rpcConfigId int) (bool, error) {
	isBlockCountHealthy := false

	previousHealthCheck, err := service.GetHealthCheckRepo().GetByRpcConfigId(rpcConfigId)
	if err != nil {
		return isBlockCountHealthy, err
	}
	...
	logger.Log(" - "+service.GetSymbol()+
		...
		", isBlockCountHealthy: "+strconv.FormatBool(isBlockCountHealthy), ctx)
